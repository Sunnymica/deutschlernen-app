...
const translations = {
  ...
  hr: {
    ...
    start: 'Idemo!',
    back: 'Natrag',
    chooseScenario: 'Odaberite scenarij:',
    dialogue: 'Dijalog',
      ...
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-md p-6 mb-6">
            ...
            <h2 className="text-3xl font-bold text-gray-800 mb-2">{t.chooseScenario}</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {scenarios.map((scenario: any) => (
              <button
                ...
                className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-all text-left"
              >
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-gray-800">{selectedScenario.title}</h2>
          </div>
        ...
        {!showVocabulary ? (
          <div className="bg-white rounded-xl shadow-md p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-4">{t.dialogue}</h3>
            <div className="space-y-3">
              {selectedScenario.dialogues.map((dialogue: any, idx: number) => (
                <div
                  key={dialogue.id}
                  className={`p-4 rounded-lg ${
                    idx % 2 === 0 ? 'bg-blue-50 mr-8' : 'bg-green-50 ml-8'
                  }`}
                >
                  <p className="text-gray-800">
                    <strong>{dialogue.speaker}:</strong> {dialogue.text}
                  </p>
                  <p className="text-gray-600 text-sm mt-1 italic">{dialogue.translation}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-4">{t.vocabulary}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedScenario.vocabulary.map((vocab: any, idx: number) => (
                <div key={idx} className="bg-gradient-to-br from-indigo-50 to-blue-50 p-4 rounded-lg border border-indigo-100">
                  <div className="font-bold text-lg text-gray-800 mb-2">{vocab.german}</div>
                  <div className="text-gray-700 mb-1">🇬🇧 {vocab.translation}</div>
                  {vocab.translationUK && (
                    <div className="text-gray-700 mb-1">🇺🇦 {vocab.translationUK}</div>
                  ))}
                  {vocab.translationHR && (
                    <div className="text-gray-700 mb-1">🇭🇷 {vocab.translationHR}</div>
                  ))}
                  <div className="text-gray-600 text-sm mt-2 italic">"{vocab.context}"</div>
                </div>
              ))}
            </div>
          </div>
